...
// Add this line to filter only rows where user_id is NULL
$whereClause .= " AND (user_id IS NULL OR user_id = 0)";
...
if ($over18 !== '') {
    // Convert "Yes"/"No" to 1/0 for database compatibility
    $over18 = strtolower(trim($over18));
    if ($over18 === 'yes') {
        $over18Int = 1;
    } elseif ($over18 === 'no') {
        $over18Int = 0;
...
if ($gender !== '') {
    $whereClause .= " AND LOWER(gender) = LOWER(%s)";  // Case-insensitive comparison
    $params[] = trim($gender);
}
...
$totalRecords = DB::queryFirstField("SELECT COUNT(*) FROM applicants");

// Total Records Count (with filtering)

$totalFilteredQuery = "SELECT COUNT(*) FROM applicants $whereClause";
...
// $dataQuery = "SELECT * FROM applicants $whereClause ORDER BY $orderColumn $orderDir LIMIT $start, $length";

// Subquery to filter applicants first
$subQuery = "SELECT * FROM applicants $whereClause";
...
// Inside the foreach loop where $data[] is constructed:
// Inside the foreach loop where $data[] is constructed:
$status = $row['status'] ?? 'N/A';
$statusDisplay = (strtolower($status) === 'hired' ? 'Hired' : 'Not Hired');
$statusClass = (strtolower($status) === 'hired' ? 'badge badge-success' : 'badge badge-danger');

$data[] = array(
    'id' => $row['id'],
    'actions' => '
        <a class="btn btn-sm" 
           href="index.php?route=modules/applicants/view_applicant_new&id='.$row['id'].'" 
           style="background-color: #fe5500; border-color: #fe5500; color: white;" 
           title="view '.htmlspecialchars($row['first_name'].' '.$row['last_name']).'">    
             <i class="fas fa-eye"></i>
        </a>'.
        ($_SESSION['role_id'] != ROLE_ID_HR ? '
        <a class="btn btn-warning btn-sm" 
           href="index.php?route=modules/applicants/edit_applicants&id='.$row['id'].'" 
        ...
        <button class="btn btn-danger btn-sm delete-btn" 
            ...
            title="Delete '.htmlspecialchars($row['first_name'].' '.$row['last_name']).'">
            <i class="fas fa-trash-alt"></i>
        </button>' : ''),
    'first_name' => $row['first_name'] ?? 'N/A',
    'last_name' => $row['last_name'] ?? 'N/A',
    'gender' => $row['gender'] ?? 'N/A',
    'dob' => !empty($row['dob']) ? date('Y-m-d', strtotime($row['dob'])) : 'N/A',
    'phone_number' => $row['phone_number'] ?? 'N/A',
    'position_name' => $row['position_name'] ?? 'N/A',
    'city' => $row['city'] ?? 'N/A',
    'state' => $row['state'] ?? 'N/A',
    'jobs' => $row['job_address'] ?? 'N/A',
    // Calculate Over 18 based on DOB if it exists, otherwise use the stored value
    'over_18' => isset($row['dob']) ? 
                 (calculateAge($row['dob']) >= 18 ? 'Yes' : 'No') : 
                 ($row['over_18'] == 1 ? 'Yes' : 'No'),
    'reference' => $row['reference'] ?? 'N/A',
    'kioskID' => $row['kioskID'] ?? 'N/A',
    'created_at' => $row['created_at'] ?? 'N/A',
    'status' => '<span class="'.$statusClass.'">'.$statusDisplay.'</span>'
);
    }
}
