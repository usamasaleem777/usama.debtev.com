...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $required = ['email', 'first_name', 'last_name', 'applicant_id'];
        ...
        $applicant_ID = htmlspecialchars($_POST['applicant_id']);
        $refference = htmlspecialchars($_POST['ReferredByName'] ?? ''); // Use ?? for optional fields
        $reference_contact = htmlspecialchars($_POST['ReferredByContact'] ?? ''); // Use ?
...
<head>
    ...
    <style type=\"text/css\">
        ...
        .header { background-color:rgb(0, 0, 0); color: white; padding: 30px 20px; text-align: center; font-size: 24px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; box-shadow: inset 0 -4px 8px rgba(0,0,0,0.15); }
...
<body>
    <div class='email-container'>
        ...
        <div class='content'>
            ...
            <p>
                A new job application has been submitted. Please find the details below:
            </p>
            <p>
                <strong>Applicant Name:</strong> {$firstName} {$lastName}<br>
                <strong>Email:</strong> {$email}<br>
                <strong>Referred By:</strong> {$refference}<br>
                <strong>Reference Contact:</strong> {$reference_contact}<br>
                <strong>Applicant ID:</strong> {$applicant_ID}
            </p>
            <p>
                <a href='https://craftgc.com/app/index.php?route=modules/applicants/view_applicant_new&id={$applicant_ID}' 
                   style='color: #fe5500; font-weight: bold; text-decoration: underline;'>
                   View Applicant Details
                </a>
            </p>
            <p>
                Kindly review the application at your earliest convenience.<br>
                If you require further information, please refer to the application system or contact the applicant directly.
            </p>
        </div>
        <div class='footer'>
...
";
        ...
        try {
            ...
            // Recipients
            $mail->setFrom('admin@craftgc.com', 'Craft Construction');
            $mail->addAddress('jobs@craftgc.com', 'hiring team');

            // Content
            $mail->isHTML(true);
            $mail->Subject = 'Packet Form Submitted - Applicant ID: ' . $applicant_ID;
            ...
            if ($mail->send()) {
                echo json_encode([
                    "status" => "success", 
                    "message" => "Notification email sent successfully to hiring team"
                ]);
            } else {
