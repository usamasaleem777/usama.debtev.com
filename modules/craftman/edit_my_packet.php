...
if (isset($_POST['save_step']) && $_POST['save_step'] == 8) {
    $applicantId = intval($_POST['applicant_ID']);
    // Lock the form
    DB::update('craft_contracting', ['is_locked' => 1], 'id=%i', $applicantId);
}
...
if (isset($_SESSION['user_id'])) {
    ...
    if ($applicantId > 0) {
        ...
        $craft_data = DB::queryFirstRow("
            ...
            WHERE craft_contracting.id = %i", $applicantId);
                // If no data in craft_contracting, try to get from applicants table
    if (!$craft_data) {
        $craft_data = DB::queryFirstRow("
            SELECT *
            FROM applicants a
            WHERE a.id = %i", $applicantId);
    }


        // Set lock status for frontend use
        $isLocked = ($craft_data && isset($craft_data['is_locked']) && $craft_data['is_locked'] == 1);
    }
    ...
    if (!$craft_data) {
        // Initialize empty craft_data instead of showing error
        $craft_data = [
            'first_name' => '',
            'last_name' => '',
            'email' => '',
    ...
    echo '<script>
        Swal.fire({
            icon: "error",
            title: "No Data Found",
            text: "Sorry, we couldn\'t find any data matching your request."
        }).then(function() {
...
<div class="d-flex justify-content-between mt-4">
    ...
    <?php if (isset($isLocked) && $isLocked): ?>
        <button type="button" id="requestEditBtn" class="btn btn-warning ms-2">
            Request Edit
        </button>
    <?php endif; ?>
    <button type="button"
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="file-upload-wrapper">
                                            <label
                                                for="licenseFront"
                                                id="frontLabel"
                                                class="license-upload fw-bold fs-5"
    ...
    button.addEventListener('click', async function (e) {
        ...
        try {
            ...
            if (currentVisibleStep === stepSequence.length - 1) {
                // Redirect to thanks.php within the module
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (requestEditBtn) {
        requestEditBtn.addEventListener('click', function(e) {
            e.preventDefault();
            Swal.fire({
                title: 'Request Edit?',
                text: 'Are you sure you want to request editing access for this form?',
                icon: 'question',
                showCancelButton: true,
                confirmButtonText: 'Yes, request edit',
                cancelButtonText: 'Cancel'
            }).then((result) => {
                if (result.isConfirmed) {
                    fetch('handle_forms.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded'
                        },
                        body: 'action=request_edit&applicant_ID=<?php echo (int)$applicantId; ?>'
                    })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            Swal.fire('Requested!', 'Your edit request has been sent.', 'success');
                            requestEditBtn.disabled = true;
                            requestEditBtn.innerText = 'Edit Requested';
                        } else {
                            Swal.fire('Error', data.message || 'Could not request edit.', 'error');
                        }
                    })
                    .catch(() => {
                        Swal.fire('Error', 'Could not request edit.', 'error');
                    });
                }
            });
        });
...
<script>
document.addEventListener('DOMContentLoaded', function() {
    var isLocked = <?php echo isset($isLocked) && $isLocked ? 'true' : 'false'; ?>;
    if (isLocked) {
        document.querySelectorAll(
            '#employmentApplication .form-step:not([data-step="1"]) input, ' +
            '#employmentApplication .form-step:not([data-step="1"]) select, ' +
            '#employmentApplication .form-step:not([data-step="1"]) textarea, ' +
            '#employmentApplication .form-step:not([data-step="1"]) button'
        ).forEach(function(el) {
            // Do NOT disable the Request Edit button
            if (
                !el.classList.contains('view-btn') &&
                !el.classList.contains('download-btn') &&
                el.id !== 'requestEditBtn'
            ) {
                el.disabled = true;
            }
        });
        ...
        msg.innerText = 'This form has been submitted and is locked. You can view but not edit except language selection.';
        var form = document.querySelector('#employmentApplication');
