...
// Handle language selection (after form submission)
if (isset($_POST['language'])) {
    $lang = $_POST['language'];
    if (in_array($lang, ['en', 'es'])) {
        $_SESSION['lang'] = $lang; // Store selected language in session
    } else {
        $_SESSION['lang'] = 'en'; // Default to English if invalid selection
    }
}
...
// Continue with normal form if language is selected
...
try {
    // Directly hardcode all form steps from 1 to 8
    $_SESSION['form_steps'] = [1, 2, 3, 4, 5, 6, 7, 8];
} catch (Exception $e) {
    error_log("Error setting form steps: " . $e->getMessage());
    // Optional fallback in case of some unexpected error
    $_SESSION['form_steps'] = [1, 2, 8];
}
...
$applicantData = [];
try {
    $applicantData = DB::queryFirstRow(
        "SELECT * FROM applicants WHERE id = %i",
        $applicantId
    );
...
if (isset($_FILES['license_file']) && $_FILES['license_file']['error'] === UPLOAD_ERR_OK) {
    ...
    $originalName = $_FILES['license_file']['name'];
    ...
    $filename = uniqid('license_', true) . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
    $destination = $uploadDir . $filename;

    // Move uploaded file
    if (move_uploaded_file($_FILES['license_file']['tmp_name'], $destination)) {
        // Save to database
        DB::update('employment_data', [
            'license_file' => $filename ?? null
        ], "id=%i", $applicantId);
    } else {
        echo "Failed to move uploaded file.";
    }
}
$_SESSION['packet_completed'] = true;
?>
...
<head>
    ...
    <div class="row">
    <div class="col-12 col-md-12 col-lg-12 col-xl-12 border rounded-3 p-4 bg-white shadow-sm" style=" overflow-y: auto;">
...
<div class="row mb-3">
    <div class="col-md-4">
        ...
        <input type="text" class="form-control bg-light" id="firstName" name="first_name" 
            value="<?= htmlspecialchars($applicantData['first_name'] ?? '') ?>">
    </div>
    <div class="col-md-4">
        ...
        <input type="text" class="form-control bg-light" id="lastName" name="last_name" 
            value="<?= htmlspecialchars($applicantData['last_name'] ?? '') ?>">
    </div>
    <div class="col-md-4">
        <label for="address" class="form-label"><?php echo lang("forms_address"); ?></label>
        <input type="text" class="form-control bg-light" name="street_address" id="address" placeholder="Enter Address"
            value="<?= htmlspecialchars($applicantData['street_address'] ?? '') ?>">
    </div>
</div>
...
<div class="row mb-3">
    <div class="col-md-4">
        ...
        <input type="text" class="form-control bg-light" name="city" id="city"
            value="<?= htmlspecialchars($applicantData['city'] ?? '') ?>">
    </div>
    ...
    <div class="col-md-4">
        ...
        <input type="number" class="form-control bg-light" name="zip_code" id="zip"
            value="<?= htmlspecialchars($applicantData['zip_code'] ?? '') ?>">
    </div>
</div>
...
<div class="col-md-4">
    ...
    <input type="tel" class="form-control" name="phone_number" id="phone_number"
           ...
           value="<?= htmlspecialchars($applicantData['phone_number'] ?? '') ?>">
</div>
    ...
    <div class="col-md-6">
        ...
        <input type="email" class="form-control bg-light" id="email" name="email" 
            ...
            value="<?= htmlspecialchars($applicantData['email'] ?? '') ?>">
        ...
        <div class="col-md-4">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <label for="account_type"
                                    class="form-label"><?php echo lang("forms_Account_1_type"); ?></label><br>
            ...
            <input type="date" class="form-control" name="date" id="date" >
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            ...
                            <button type="button"
                                class="btn btn-primary next-step"><?php echo lang("forms_next"); ?></button>
                        </div>
                    </div>
                <?php endif; ?>
    ...
    <input type="text" class="form-control" id="ssn" name="ssn" placeholder="XXX-XX-XXXX" required
        pattern="^\d{3}-\d{2}-\d{4}$" title="Format: XXX-XX-XXXX (only numbers allowed)"
        maxlength="11">
</div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-control" name="marital_status" id="marital_status">
                                    ...
                                    <option value="Single or Married filing separately"><?php echo lang("forms_Single_or_Married_filing_separately"); ?></option>
                                    <option value="Married filing jointly or Qualifying surviving spouse"><?php echo lang("forms_Married_filing_jointly_or_Qualifying_surviving_spouse"); ?></option>
                                    <option value="Head of Household"><?php echo lang("forms_Head_of_Household"); ?></option>
...
<h6 class="mb-0"><?php echo lang("forms_I agree that : under penalty of perjury,_that to_the best of_my_knowledge, this employee is_authorized to work_in the United States, and if the employee presented document(s), the document(s) I have examined appear to be genuine and to relate to the individual."); ?></h6> -->
            ...
            <input type="text" class="form-control" name="employee_telephone" id="employee_telephone">
                    ...
                    <div class="row">
                        <div class="col-md-6  form-check-inline">
                            ...
                            <label class="form-check-label"
                                for="citizen_of_us"><?php echo lang("forms_A Citizen_Of_United_States"); ?></label>
                        </div>
                        <div class="col-md-6  form-check-inline">
                            <input class="form-check-input" type="radio" name="citizenship_status" id="noncitizen_of_us"
                                value="Non U.S Citizen">
                            <label class="form-check-label"
                                for="noncitizen_of_us"><?php echo lang("forms_A Noncitizen Of United States"); ?></label>
                        </div>
                        <div class="col-md-12  form-check-inline">
                            ...
                            <label class="form-check-label"
                                for="lawful_permanent_resident"><?php echo lang("forms_A lawful permanent resident_(Alien Registration Number/USCIS Number):"); ?></label>
                        </div>
                        <div class="col-md-12  form-check-inline">
                            <input class="form-check-input" type="radio" name="citizenship_status"
                                id="allen_authorized_work" value="Allen Authorized Worker">
                            <label class="form-check-label"
                                for="allen_authorized_work"><?php echo lang("forms_An alien authorized to work_until_(expiration date, if applicable):"); ?></label>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" class="form-control" name="expiration_date" id="expiration_datee">
                        </div>
                        <div class="col-md-4">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="file-upload-wrapper">
                                            <div class="upload-options">
                                                <input type="file" class="license-upload" name="license_front"
                                                    id="licenseFront" accept="image/*,application/pdf" capture="environment"
                                                    onchange="previewLicense(event, 'frontPreview')">
                                                <button type="button" class="btn btn-sm btn-outline-primary mt-2"
                                                    onclick="capturePhoto('front')">
                                                    <i class="fas fa-camera"></i> <?php echo lang("forms_Take_Photo"); ?>
                                                </button>
                                            </div>
                            ...
                            <div class="col-md-6 mb-4">
                                <div class="card h-100">
                                    <div class="card-body">
                                        ...
                                        <div class="file-upload-wrapper">
                                            <div class="upload-options">
                                                <input type="file" class="license-upload" name="license_back"
                                                    id="licenseBack" accept="image/*,application/pdf" capture="environment"
                                                    onchange="previewLicense(event, 'backPreview')">
                                                <button type="button" class="btn btn-sm btn-outline-primary mt-2"
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <div class="non-compete-content mb-4" style="border: 1px solid #ddd; padding: 20px;">
                                    <p>This Non-Compete Agreement ("Agreement") is entered into on this <?php echo date('j'); ?> day of <?php echo date('F'); ?>, <?php echo date('Y'); ?>, by and between:</p>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control bg-light"
                                                    value="<?= htmlspecialchars($applicantData['first_name'] . ' ' . $applicantData['last_name'] ?? '') ?>"
                                                    readonly>
                                            </div>
                                            <div class="form-group">
                                                <label>Address:</label>
                                                <input type="text" class="form-control" value="<?= htmlspecialchars($applicantData['street_address'] ?? '') ?>"
                                                    readonly="">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="agreement-text">
                                        <h6>1. Purpose</h6>
                                        <p>The purpose of this Agreement is to protect the legitimate business interests of the Employer, including its confidential methods, trade secrets, customer relationships, and specialized experience related to the installation, dismantling, or servicing of Sprung structures.</p>

                                        <h6>2. Non-Compete Obligation</h6>
                                        <p>For a period of three (3) years following the termination of Employee's employment with Employer, whether voluntary or involuntary, the Employee shall not, directly or indirectly:</p>
                                        <ul>
                                            <li>Perform any work, services, or provide labor (as an employee, independent contractor, consultant, or otherwise) for any entity or individual involved in the installation, dismantling, service, or construction of Sprung brand structures, within the state of Ohio or any state in which Craft Contracting, LLC has an active project at the time of termination.</li>
                                        </ul>

                                        <h6>3. Confidentiality</h6>
                                        <p>Employees agree not to disclose or use any confidential or proprietary information obtained during their employment, including but not limited to project procedures, pricing, client lists, and material specifications, especially as it relates to Sprung structures.</p>

                                        <h6>4. Acknowledgment</h6>
                                        <p>Employee acknowledges that this restriction is reasonable in scope and necessary to protect the Employer's interests, and that employment with Employer constitutes sufficient consideration for entering this Agreement.</p>

                                        <h6>5. Governing Law</h6>
                                        <p>This Agreement shall be governed by and construed in accordance with the laws of the State of Ohio.</p>

                                        <h6>6. Enforcement</h6>
                                        <p>If any provision is deemed unenforceable, the remaining provisions shall remain in effect. Employer shall be entitled to injunctive relief, in addition to any other legal remedies, in the event of a breach.</p>

                                        <br>
                                        <p><strong>IN WITNESS WHEREOF, the parties have executed this Agreement as of the date written below.</strong></p>
                                    ...
                                    <div class="row mb-4">
                        <!-- <div class="col-md-6">
                            <label class="form-label"><?php echo lang("form_signature"); ?><span
                                    class="text-danger">*</span></label>
                        <div style="border: 1px solid #ddd; border-radius: 4px; margin-bottom: 10px;">
                                <canvas id="signaturePad" width="400" height="200" style="width: 100%;"></canvas>
                            </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Auto-save signature when drawing stops (mouseup or touchend for mobile support)
        canvas.addEventListener('mouseup', saveSignature);
        canvas.addEventListener('touchend', saveSignature); // For mobile devices

        // Function to save the signature data in the hidden input field
        ...
        // Event listener for the "Next" button in data step 7 (or any other step)
        const nextButton = document.querySelector('.next-step'); // This assumes "Next" button has class "next-step"
        if (nextButton) {
            nextButton.addEventListener('click', function (e) {
                // Check if we are on data step 7 and if signature exists
                const currentStep = document.querySelector('.step-7'); // Adjust based on the structure of your multi-step form
                if (currentStep && signaturePad.isEmpty()) {
                    // Prevent moving to next step if signature is empty
                    alert('<?php echo lang("form_provide_signature"); ?>');
                    e.preventDefault();
                    return false;
                }

                // Ensure the signature data is stored in the hidden input field before navigating
                if (!signatureInput.value) {
                    const signatureData = signaturePad.toDataURL('image/png');
                    signatureInput.value = signatureData;
                }

                // Proceed to next step if signature is saved
                // Optionally, you can add logic to dynamically show/hide steps based on your form structure
            });
        }
                ...
                <p>
                    ...
                    <div class="row mb-4">
                        <div class="col-12">
                            ...
                            <div class="g-recaptcha" data-sitekey="6LfNWxMrAAAAACGjxGAk224sWcMicyPk23x1QeBx"></div>
...
$(document).ready(function() {
    ...
    return isValid;
        ...
        async function submitStepData(targetStep) {
            ...
            try {
                const response = await fetch('handle_forms.php', {
                    method: 'POST',
                    body: formData
                });
                ...
                if (targetStep === 8) {
                    window.location.href = 'index.php?route=modules/craftman/view_my_packet';
                }
                return result;
            } catch (error) {
    ...
    function startCamera() {
        navigator.mediaDevices.getUserMedia({
            ...
            .catch(error => {
                ...
                alert('<?php echo lang("forms_Camera_access_denied"); ?>');
            });
    }
