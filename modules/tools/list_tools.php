        ...
        <div class="page-header d-flex align-items-center justify-content-end mt-2 mb-2">
            <div>
                <ol class="breadcrumb float-sm-right mt-2">
                    ...
                    <!-- View tools breadcrumb -->
                    <li class="breadcrumb-item">
                        <a href="index.php?route=modules/admin/tool-management/view_tool"
                            style="color: #fe5500"><?php echo lang("tools_Tools"); ?></a>
                    </li>
                </ol>
            </div>
        </div>
            ...
            <div class="card border-top" style="border-color: #FE5500 !important;">
                <div class="card-body p-4">
                    ...
                    <div class="d-flex align-items-start justify-content-between mb-3">
                        <div>
                            <h5 class="mb-0 fw-bold" style="color: #FE5500;"><?php echo lang("tools_all_Tools"); ?></h5>
                        </div>
                    </div>
                        ...
                        <table class="table table-striped table-hover border text-nowrap mb-0 datatable mt-3 w-100"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th style="background-color: #FE5500; color: white;"><?php echo lang("tools_image"); ?></th>
                                    <th style="background-color: #FE5500; color: white;">
                                        <?php echo lang("tools_Tool_name"); ?>
                                    </th> <!-- Role title -->
                                    <th style="background-color: #FE5500; color: white;">
                                        <?php echo lang("tools_Description"); ?>
                                    </th>
                                    <th style="background-color: #FE5500; color: white;">
                                        <?php echo lang("tools_Quantity"); ?>
                                    </th> <!-- Role title -->
                                    <th class="text-center" style="background-color: #FE5500; color: white;">
                            ...
                            <tbody>
                                ...
                                try {
                                    ...
                                    if (!empty($tools)) {
                                        ...
                                        foreach ($tools as $itr => $tool) {
                                ?>
                                            <tr>
                                                <td class="text-center"><?php echo ($itr + 1); ?></td> <!-- Row number -->
                                                <td>
                                                    <?php if (!empty($tool['tool_picture'])): ?>
                                                        <img src="uploads/tools/<?php echo htmlspecialchars($tool['tool_picture']); ?>"
                                                            alt="Tool Image" width="60" height="60" style="object-fit: cover; border-radius: 5px;" />
                                                    <?php else: ?>
                                                        <span class="text-muted">No Image</span>
                                                    <?php endif; ?>
                                                </td>
                                                <td><?php echo htmlspecialchars($tool['tool_name']); ?></td> <!-- Role name -->
                                                <td><?php echo htmlspecialchars($tool['tool_description']); ?></td> <!-- Role name -->
                                                <td><?php echo htmlspecialchars($tool['quantity']); ?></td> <!-- Role name -->
                                                <td class="text-center">
                                                    <!-- Edit button - links to edit page with role ID and name -->
                                                    <a href="index.php?route=modules/tools/edit_tool&tool_id=<?php echo $tool['tool_id']; ?>&tool_name=<?php echo urlencode($tool['tool_name']); ?>"
                                                        class="btn edit_btn" title="<?php echo lang("tool_edit"); ?>"
                                                    ...
                                                    <button type="button" onclick="confirmDeleteRole(<?php echo $tool['tool_id']; ?>)"
                                                        class="btn btn-danger ms-2" title="<?php echo lang("role_delete"); ?>"
                                        ...
                                        // Display message if no tools found
                                        ?>
                                        <tr>
                                            <td colspan="3" class="text-center"><i
                                                    class="fas fa-info-circle me-2"></i><?php echo lang("tools_No Tools Found"); ?>
                                            </td>
                                        </tr>
                                    <?php
                                    }
                                    ...
                                    ?>
                                    <tr>
                                        <td colspan="3" class="text-center text-danger"><i
...
<script>
    $(document).ready(function() {
        ...
        const table = $('#applicants_table').DataTable({
            ...
            "language": {
                "emptyTable": "No tools available" // Message for empty table
            },
            "initComplete": function() {
                // On mobile, reposition the length and filter controls
                if (window.innerWidth <= 576) {
                    setTimeout(function() {
                        $('#applicants_table_length').appendTo('#mobileLengthContainer').css(
                            'display', 'inline-block');
                        $('#applicants_table_filter').appendTo('#mobileSearchContainer').css(
                            'display', 'inline-block');
        ...
        $(window).resize(function() {
            if (window.innerWidth <= 576) {
                ...
                // Desktop styles
                $('#applicants_table_filter input[type="search"]').css('float', 'none').css('width',
                    'auto');
    ...
    function confirmDeleteRole(roleId) {
        Swal.fire({
            ...
            text: "Are you sure you want to delelte this tool?", // Warning message
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'index.php?route=modules/tools/processtoolForm',
                    type: 'POST',
                    data: {
                        tool_id: roleId,
                        btnDeletetool: true
