...
?>
    ...
    <head>
        ...
        <style>
            ...
            .table-hover tbody tr:hover {
                ...
                #assignToolsModal .form-check-input {
        width: 2.5em;
        height: 2.5em;
    }
        </style>
    ...
    <h5 class="card-title mb-0 text-white">
        ...
        <?php echo lang("History"); ?>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    <thead class="bg-primary text-white">
                                        <tr>
                                            <th><?php echo lang("tools_Tool_Name"); ?></th>
                                            <th><?php echo lang("tools_Picture"); ?></th>
                    ...
                    <!-- Assign Tools Modal -->
                    <div class="modal fade" id="assignToolsModal" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                <form id="assignToolsForm">
                                    <div class="modal-header">
                                        <h5 class="modal-title">
                                            <i class="fas fa-tools me-2"></i>
                                            <?php echo lang("Assign Tools"); ?>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-borderless">
                                                <thead>
                                                    <tr>
                                                        <th></th>
                                                        <th><?php echo lang("tool name"); ?></th>
                                                        <th><?php echo lang("picture"); ?></th>
                                                        <th><?php echo lang("available"); ?></th>
                                                        <th><?php echo lang("assign quantity"); ?></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <?php
                                                    $tools = DB::query("SELECT tool_id, tool_name, tool_picture, quantity FROM tools ORDER BY tool_name ASC");
                                                    foreach ($tools as $tool): ?>
                                                        <tr class="tool-row align-middle">
                                                            ...
                                                            <td>
                                                                <img src="uploads/tools/<?= $tool['tool_picture'] ?>"
                                                                    class="img-thumbnail" style="width: 60px; height: 60px;">
                                                            </td>
                                                            <td class="available-quantity"><?= $tool['quantity'] ?></td>
                                                            <td>
                                                                <input type="number"
                                                                    class="form-control tool-quantity"
                                                                    min="0"
                                                                    max="<?= $tool['quantity'] ?>"
                                                                    name="tools[<?= $tool['tool_id'] ?>]"
        ...
        <script>
            $(document).ready(function() {
                ...
                function refreshToolQuantities() {
                    $.ajax({
                        url: 'ajax_helpers/ajax_get_tools.php',
                        method: 'GET',
                        success: function(response) {
                            if (response.success) {
                                response.tools.forEach(tool => {
                                    const $row = $(`.tool-row input[name="tools[${tool.tool_id}]"]`).closest('tr');
                                    $row.find('.available-quantity').text(tool.quantity);
                                    $row.find('.tool-quantity')
                                        .attr('max', tool.quantity)
                                        .val(0);
                                });
                            }
                        }
            ...
            `);
                    ...
                    $.ajax({
                        url: 'ajax_helpers/ajax_user_tool_history.php',
                        method: 'POST',
                        data: {
            ...
            `);
                ...
                $(document).on('click', '.checkin-btn', function() {
                    ...
                    Swal.fire({
                        ...
                        text: `<?php echo lang("confirm_checkin_text"); ?> ${quantity}?`,
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: 'ajax_helpers/ajax_checkin_tool.php',
                                method: 'POST',
                                data: {
                                    user_id: currentUserId,
                                    tool_id: toolId,
                                ...
                                success: function(response) {
                                    if (response.success) {
                                        ...
                                        // Refresh data instead of reloading
                                        loadAssignmentHistory();
                ...
                $('#assignToolsForm').submit(function(e) {
                    e.preventDefault();
                    // Check if signature exists
                    if (!currentUser?.signature || !currentUser.signature.startsWith('data:image')) {
                        showAlert('error', '<?php echo lang("no_signature_error"); ?>');
                        return;
                    }
                    ...
                    if (Object.keys(formData.tools).length === 0) {
                        showAlert('warning', '<?php echo lang("select_tools_warning"); ?>');
                        $('.tool-row input[type="checkbox"]').addClass('is-invalid');
                        return;
                    }

                    // Add validation for quantity exceeding available
                    let valid = true;
                    $('.tool-row').each(function() {
                        const $row = $(this);
                        const max = parseInt($row.find('.tool-quantity').attr('max'));
                        const entered = parseInt($row.find('.tool-quantity').val()) || 0;

                        if (entered > max) {
                            $row.find('.tool-quantity').addClass('is-invalid');
                            valid = false;
                        }
                    });

                    if (!valid) {
                        showAlert('error', '<?php echo lang("quantity_exceeded"); ?>');
                        return;
                    }

                    $.ajax({
                        url: 'ajax_helpers/user_assign_tools.php',
                        method: 'POST',
                        data: formData,
                        success: function(response) {
                            if (response.success) {
                                $('#assignToolsModal').modal('hide');
                                showAlert('success', response.message);
                                // Refresh both history and user info
                                loadAssignmentHistory();
                                refreshToolQuantities();
                                fetchUserDetails(currentUserId);
                            } else {
                                ...
                                $('.tool-row').each(function() {
                                    const $row = $(this);
                                    const toolId = $row.find('.tool-quantity').data('tool-id');
                                    if (response.message.includes(toolId)) {
                ...
                $(document).on('show.bs.modal', '#assignToolsModal', function() {
                    refreshToolQuantities();
                ...
                function createHistoryRow(assignment) {
                    ...
                    const showSignature = !assignment.chekin_at;
                    const checkinDate = assignment.chekin_at ?
                        new Date(assignment.chekin_at).toLocaleDateString() :
                        `<button class="btn btn-sm btn-primary checkin-btn"
                            data-tool-id="${assignment.tool_id}"
                            data-quantity="${assignment.assigned_quantity}">
                            <i class="fas fa-sign-in-alt me-2"></i>
                            <?php echo lang("checkin"); ?>
                        </button>`;
                    
                    const signatureCell = showSignature ?
                    ...
                    return `
                        <tr>
                            ...
                            <td>
                                <img src="uploads/tools/${assignment.tool_picture}" 
                                     class="img-thumbnail" 
                ...
                function gatherFormData() {
                    ...
                    $('.tool-row').each(function() {
                        ...
                        const quantity = parseInt($row.find('.tool-quantity').val()) || 0;
